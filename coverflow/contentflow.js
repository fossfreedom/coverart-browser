...
/*
 * ============================================================
 * lets pass messages to python
 * ============================================================
 */
function message_signal() {
    // create a JSON string from the args passed
    // we assume the first arg is the signal name
    var obj = { }
    obj['signal'] = arguments[0];
    var args = new Array();
    
    for (var i = 1; i < arguments.length; i++) {
        args[i-1] = String(arguments[i]);
    }
    obj['param'] = args;
    
    document.title = JSON.stringify(obj);
    document.title = JSON.stringify('clear'); // clear title for next call
}

/*
 * ============================================================
 * our callback from python
 * ============================================================
 */
function jscallback(msg) {
    ...
    /* now create rows dynamically for data passed*/
    for (var i=0;i < obj.album.length; i++) {

        var newJob = document.createElement("tr");
        newJob.innerHTML = "<td>"+obj.album[i].track+"</td><td>" +
            obj.album[i].artist + "</td>";
...
function new_flow_batch(msg) {
    ...
    if (position != 'stop') {
        for (var i=0; i < obj.flowbatch.length; i++) {
...
var ContentFlowItem  = function (CFobj, element, index) {
    ...
    this.rightclickItem = function (event) {
        if(!event) var event = window.event;
    ...
    this.dblclickItem = function (event) {
        if(!event) var event = window.event;
        var el = event.target ? event.target : event.srcElement;
        var index = el.itemIndex ? el.itemIndex : el.parentNode.itemIndex;
        var item = this.items[index];
        if (this._activeItem == item) {
            this.conf.ondblclickActiveItem(item);
        }
    }.bind(CFobj),
